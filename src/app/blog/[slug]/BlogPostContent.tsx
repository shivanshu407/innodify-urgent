    ...
    return (
        <section className="relative min-h-screen bg-[#0e1012] py-24 overflow-hidden">
            ...
            <div className="container mx-auto px-6 max-w-4xl relative z-10">
                <Link href="/blog" className="inline-flex items-center gap-2 text-[#9ca3af] hover:text-[#00adef] transition-colors mb-12 group">
                    <ArrowLeft size={20} className="group-hover:-translate-x-1 transition-transform" />
                    Back to Insights
                </Link>

                <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
                    <span className="inline-block px-4 py-1 rounded-full bg-[#00adef]/10 text-[#00adef] text-sm font-medium mb-6">{post.category}</span>
                    ...
                    <div className="flex flex-wrap items-center gap-6 mb-12 py-8 border-y border-[#2a2f36]">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#00adef] to-blue-600 flex items-center justify-center text-white font-bold">{post.author.charAt(0)}</div>
                            <div>
                        ...
                        <div className="flex items-center gap-6 text-sm text-[#9ca3af]">
                            <span className="flex items-center gap-2"><Calendar size={16} className="text-[#00adef]" />{post.date}</span>
                            <span className="flex items-center gap-2"><Clock size={16} className="text-[#00adef]" />{post.readTime}</span>
                        </div>
                    </div>
                    ...
                    <div className="mt-16 pt-12 border-t border-[#2a2f36] flex flex-col md:flex-row md:items-center justify-between gap-8">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex gap-2">
                                <button className="p-3 rounded-lg bg-[#15181c] text-[#9ca3af] hover:text-[#00adef] hover:bg-[#00adef]/10 transition-all border border-[#2a2f36]"><Facebook size={20} /></button>
                                <button className="p-3 rounded-lg bg-[#15181c] text-[#9ca3af] hover:text-[#00adef] hover:bg-[#00adef]/10 transition-all border border-[#2a2f36]"><Twitter size={20} /></button>
                                <button className="p-3 rounded-lg bg-[#15181c] text-[#9ca3af] hover:text-[#00adef] hover:bg-[#00adef]/10 transition-all border border-[#2a2f36]"><Linkedin size={20} /></button>
                                <button className="p-3 rounded-lg bg-[#15181c] text-[#9ca3af] hover:text-[#00adef] hover:bg-[#00adef]/10 transition-all border border-[#2a2f36]"><Share2 size={20} /></button>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {recommendedPosts.map((rPost, idx) => (
                            <Link key={idx} href={`/blog/${rPost.slug}`} className="group">
                                ...
                                <h3 className="mt-2 text-lg font-semibold text-white group-hover:text-[#00adef] transition-colors line-clamp-2">{rPost.title}</h3>
